...
public class ArcadeApp extends Application {
	...
	Image image = new Image("file:Retro-80s-Arcade-Game.jpg");
	...
	// button and image on button 2 
	Button button2 = new Button();
	button2.setOnAction((event) -> {
		Reversi game2 = new Reversi();
		game2.game();
	    });
	
	Image reversiImage = new Image("file:Reversi.jpg");
	ImageView reversiView = new ImageView(reversiImage);
	reversiView.setFitHeight(160);
	reversiView.setFitWidth(160);
	button2.setGraphic(reversiView);
	...
	Label intro = new Label("\nTim Xu \t Rory Donahue \nTeam: CROSSING STREAMS");
	intro.setTextFill(Color.ANTIQUEWHITE);
	VBox intro2 = new VBox();
	intro2.getChildren().addAll(intro);
	intro2.setAlignment(Pos.TOP_CENTER);
	stackpane2.getChildren().addAll(loading, intro2);
